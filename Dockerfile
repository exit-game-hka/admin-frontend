# ENVIRONMENT VARS
ARG NODE_VERSION=21.1.0

# Install dependencies
FROM node:${NODE_VERSION}-alpine AS deps
# RUN apk add --no-cache libc6-compat
WORKDIR /app
COPY package.json package-lock.json ./
RUN npm ci

# Build the app
FROM node:${NODE_VERSION}-alpine AS builder
WORKDIR /app
COPY --from=deps /app/node_modules ./node_modules
COPY . .

ARG NEXT_PUBLIC_APPLICATION_NAME
ARG NEXT_PUBLIC_BASE_PATH
ARG NEXT_PUBLIC_BACKEND_URL
ARG NEXT_PUBLIC_APP_VERSION
ARG NEXT_PUBLIC_BUILD_NUMBER
ARG NEXT_PUBLIC_DEMO_FRONTEND_CLIENT_ID
ARG NEXT_PUBLIC_DEMO_FRONTEND_CLIENT_SECRET
ARG NEXT_PUBLIC_AUTH_ISSUER
ARG NEXT_PUBLIC_REFRESH_TOKEN_URL
ARG AUTH_SECRET
ARG NEXTAUTH_URL
ARG NEXTAUTH_SECRET
ARG NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_SERVER_URL
ARG NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_RECONNECTION_DELAY
ARG NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_INCOMING_HEARTBEAT
ARG NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_OUTGOING_HEARTBEAT
ARG NODE_OPTIONS
ARG NODE_TLS_REJECT_UNAUTHORIZED

ENV NEXT_PUBLIC_APPLICATION_NAME=$NEXT_PUBLIC_APPLICATION_NAME
ENV NEXT_PUBLIC_BASE_PATH=$NEXT_PUBLIC_BASE_PATH
ENV NEXT_PUBLIC_BACKEND_URL=$NEXT_PUBLIC_BACKEND_URL
ENV NEXT_PUBLIC_APP_VERSION=$NEXT_PUBLIC_APP_VERSION
ENV NEXT_PUBLIC_BUILD_NUMBER=$NEXT_PUBLIC_BUILD_NUMBER
ENV NEXT_PUBLIC_DEMO_FRONTEND_CLIENT_ID=$NEXT_PUBLIC_DEMO_FRONTEND_CLIENT_ID
ENV NEXT_PUBLIC_DEMO_FRONTEND_CLIENT_SECRET=$NEXT_PUBLIC_DEMO_FRONTEND_CLIENT_SECRET
ENV NEXT_PUBLIC_AUTH_ISSUER=$NEXT_PUBLIC_AUTH_ISSUER
ENV NEXT_PUBLIC_REFRESH_TOKEN_URL=$NEXT_PUBLIC_REFRESH_TOKEN_URL
ENV AUTH_SECRET=$AUTH_SECRET
ENV NEXTAUTH_URL=$NEXTAUTH_URL
ENV NEXTAUTH_SECRET=$NEXTAUTH_SECRET
ENV NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_SERVER_URL=$NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_SERVER_URL
ENV NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_RECONNECTION_DELAY=$NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_RECONNECTION_DELAY
ENV NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_INCOMING_HEARTBEAT=$NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_INCOMING_HEARTBEAT
ENV NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_OUTGOING_HEARTBEAT=$NEXT_PUBLIC_WEBSOCKET_NOTIFICATIONS_OUTGOING_HEARTBEAT
ENV NODE_OPTIONS=$NODE_OPTIONS
ENV NODE_TLS_REJECT_UNAUTHORIZED=$NODE_TLS_REJECT_UNAUTHORIZED
ENV NEXT_TELEMETRY_DISABLED=1

RUN npm run build

# Run the app
FROM node:${NODE_VERSION}-alpine AS runner
WORKDIR /app

ENV NODE_ENV=production
ENV NEXT_TELEMETRY_DISABLED=1

RUN addgroup --system --gid 1001 nodejs
RUN adduser --system --uid 1001 nextjs

COPY --from=builder /app/public ./public
COPY --from=builder /app/package.json ./package.json

# https://nextjs.org/docs/advanced-features/output-file-tracing
COPY --from=builder --chown=nextjs:nodejs /app/.next/standalone ./
COPY --from=builder --chown=nextjs:nodejs /app/.next/static ./.next/static

USER nextjs

#EXPOSE 3001

ENV PORT 3000

CMD ["node", "server.js"]
